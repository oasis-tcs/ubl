...
<xs:doc info="$Id: ndrSubset.xsl,v 1.35 2016/05/29 16:47:02 admin Exp $"
...
<xsl:template name="gu:checkSubsetting">
  <xsl:choose>
    ...
    <xsl:when test="string($subset-column-name) and
                  ( every $gu:subset in 
        ($gu:gc,$gu:gcOther)/*/SimpleCodeList/Row/gu:col(.,$subset-column-name)
                    satisfies normalize-space($gu:subset)='' )">
...
<xsl:key name="gu:bie-by-class-and-type" match="Row" 
