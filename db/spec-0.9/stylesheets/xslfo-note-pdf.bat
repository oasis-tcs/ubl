...
set xml-pdf-out=%~f3
pushd %~dp0
echo java -cp ".;saxon.jar;ibex-crane-ss.jar" "-Dibexconfig=ibexconfig.xml" ibex.Signed -xml "%xml-pdf-in%" -xsl "oasis-note-fo-%~2.xsl" -manifest "oasis-note-fo-manifest.xml" -pdf "%xml-pdf-out%"
java -cp ".;saxon.jar;ibex-crane-ss.jar" "-Dibexconfig=ibexconfig.xml" ibex.Signed -xml "%xml-pdf-in%" -xsl "oasis-note-fo-%~2.xsl" -manifest "oasis-note-fo-manifest.xml" -pdf "%xml-pdf-out%"
popd
set xml-pdf-in=
set xml-pdf-out=
