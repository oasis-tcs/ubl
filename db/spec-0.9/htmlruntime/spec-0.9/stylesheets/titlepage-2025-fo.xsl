...
<!-- This stylesheet is hacked from an earlier version, rather than generated
     because the original generation template cannot be found-->
...
-->  
  ...
  <xsl:apply-templates mode="article.titlepage.recto.auto.mode" select="articleinfo/legalnotice[not(normalize-space(@condition) and                                    not(contains(@condition,'oasis')))]                               [@role = 'namespaces']"/>
  <xsl:apply-templates mode="article.titlepage.recto.auto.mode" select="artheader/legalnotice[not(normalize-space(@condition) and                                    not(contains(@condition,'oasis')))]                               [@role = 'namespaces']"/>
  <xsl:apply-templates mode="article.titlepage.recto.auto.mode" select="info/legalnotice[not(normalize-space(@condition) and                                    not(contains(@condition,'oasis')))]                               [@role = 'namespaces']"/>
...
<xsl:template name="article.titlepage.before.recto"><fo:block xmlns:fo="http://www.w3.org/1999/XSL/Format" text-align="start">
      <fo:external-graphic src="url(&#34;{$oasis.logo}&#34;)" content-width="85mm" width="85mm"/>
    </fo:block>
</xsl:template>
