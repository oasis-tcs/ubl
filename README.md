...
The outer ZIP is used to wrap the Action’s artifacts results. The inner ZIP wraps the work product results suitable for posting to the OASIS Kavi server for archive purposes: the set of work product files to be posted to the OASIS Docs server, and a ZIP of the supplemental files used in the production process for archive purposes according to OASIS TC obligations. These Action results are transient and not placed into the GitHub code repository. After 90 days GitHub deletes old Action results. 
